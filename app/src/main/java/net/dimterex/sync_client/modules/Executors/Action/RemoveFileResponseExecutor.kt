...
class RemoveFileResponseExecutor(private val fileManager: FileManager, private val _eventLoggerManager: EventLoggerManager) : IExecute<RemoveFileResponce> {
    override fun Execute(param: RemoveFileResponce) {
        var filePath = fileManager.getFullPath(param.file_name, true)
        _eventLoggerManager.save_event(EventDto(param.file_name, "was removed"))
    }
}